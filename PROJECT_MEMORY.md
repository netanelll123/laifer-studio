...
- **Entity graph is deliberately cross-linked, and every `@id` is locale-invariant.** Rule: an `@id` identifies the *entity* (the video, the person, the site) and must stay byte-identical in every document that mentions it, regardless of locale; only leaf properties like `name`/`description`/`url` are allowed to vary per language. `url` and `@id` are therefore two different fields with two different jobs — `url` is this specific document's address (correctly `/he/work/x` on one page and `/en/work/x` on the other), `@id` is the shared identity (must NOT include the locale segment, or the same real-world work silently becomes two different entities on the two locale pages, which defeats the point of linking them at all). `lib/json-ld.ts` has one function per concept: `caseStudyUrl(locale, slug)` for the per-locale page URL, `caseStudyId(slug)` for the locale-invariant identity, `serviceId(slug)` likewise for Services, and `siteIds.faq`/`siteIds.portfolio` for the homepage's FAQPage/ItemList. Verified empirically (not just by reasoning) after the initial implementation surfaced exactly this bug: script in the conversation history parsed every rendered page's JSON-LD across both locales and confirmed (a) `Person`/`Organization`/`WebSite` each have exactly one `@id` site-wide, (b) every case study's `@id` is now identical between its `/he` and `/en` page, the homepage `ItemList`, and `WebSite.hasPart`, (c) zero orphaned `{"@id": ...}` references (every reference resolves to a real declaration), (d) the same `@id` never appears with conflicting `@type`s (the homepage `ItemList` entry for a video-backed case study matches `["CreativeWork","VideoObject"]`, same as that page's own node — plain `CreativeWork` in `WebSite.hasPart`'s lightweight stub is fine since VideoObject *is a* CreativeWork, not a contradiction, just less specific). Concretely wired: `WebSite.hasPart` ↔ each case study's `isPartOf`; `Organization.subjectOf` (the FAQPage) ↔ that `FAQPage.about`; a case study's `creator` list resolves Netanel's own credit to `{"@id": siteIds.person}` (matched against the locale-aware `metadata.personName`, not a hardcoded string) instead of a second anonymous Person. `Person.sameAs` includes the real Wikipedia article — belongs on Person, not Organization, since the article is about him, not the studio brand. When adding anything new that references an existing entity: wire it to the existing `@id`, and if that `@id` will appear on more than one locale's page, make sure it's minted without a locale segment.
