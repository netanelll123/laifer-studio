...
- **Entity graph is deliberately cross-linked, not just individually valid.** Every reference to "the same real-world thing" uses the identical `@id`/URL everywhere it appears, so a crawler (Google or an AI answer engine) can resolve them as one entity instead of several look-alike stubs: `caseStudyUrl(locale, slug)` from `lib/json-ld.ts` is both a case-study `CreativeWork`'s own `@id`/`url`, the `@id` used for that same work inside the homepage `ItemList`, and the `@id` used inside `WebSite.hasPart`. Concretely: `WebSite.hasPart` ↔ each case study's `isPartOf` (bidirectional); `Organization.subjectOf` (the homepage FAQPage) ↔ that same `FAQPage.about` (bidirectional, matched by the `@id` `absoluteUrl(/${locale}#${sectionIds.faq})`); a case study's `creator` list resolves Netanel's own credit entry to `{"@id": siteIds.person}` instead of restating him as a second anonymous Person — matched by comparing `credit.name` against `metadata.personName` (the same localized display name used elsewhere), not a hardcoded string, so it works in both locales. `Person.sameAs` includes the real Wikipedia article (`siteConfig.person.wikipedia`) — that belongs on Person, not Organization, since the article is about him, not the studio brand. When adding new content that references an existing entity, prefer wiring it to the existing `@id` over restating fields — that's the pattern to extend, not a one-off.
